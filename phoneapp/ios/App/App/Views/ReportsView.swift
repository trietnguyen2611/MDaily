...
public struct ReportsView: View {
    ...
    private var analyticsSection: some View {
        ...
        let categoryTotals: [(item: CategoryItem, total: Double, color: Color)] = store.categories.enumerated().compactMap { idx, cat in
    ...
    // MARK: - Category Manager Section
    private var categoryManagerSection: some View {
        VStack(spacing: 16) {
            // Add Category Card
            if !isAddingCategory {
                Button {
            ...
            // Category List
            VStack(spacing: 12) {
                let categoriesList: [CategoryItem] = store.categories
                ForEach(categoriesList, id: \.id) { (cat: CategoryItem) in
                    let count: Int = expenses.filter { $0.category == cat.id }.count
                    let catTotal: Double = expenses.filter { $0.category == cat.id }.reduce(0.0) { $0 + $1.amount }

                    if editingCategoryId == cat.id {
                        categoryEditRow(for: cat)
                    } else {
                        categoryDisplayRow(for: cat, count: count, total: catTotal)
                    }
                }
            }
        }
    ...
    private func categoryEditRow(for cat: CategoryItem) -> some View {
        HStack(spacing: 10) {
            ...
            Button(store.t("save")) {
                ...
                if !trimmed.isEmpty {
                    store.updateCategory(id: cat.id, newLabel: trimmed)
    ...
    @ViewBuilder
    private func categoryDisplayRow(for cat: CategoryItem, count: Int, total: Double) -> some View {
        HStack {
            HStack(spacing: 12) {
